...
class Model(nn.Module): #Trompt
    ...
    def forward(self, x_num : None | Tensor = None, x_bin : None | Tensor = None, x_cat : None | Tensor = None):
        ...
        return F.logsigmoid(torch.stack(outputs, dim=1).squeeze(-1))
...
def main(
    ...
    loss_fn = F.binary_cross_entropy#lib.deep.get_loss_fn(dataset.task.type_)
            ...
            while eval_batch_size:
                try:
                    predictions[part] = (
                        torch.cat(
                            [
                                apply_model(part, idx).mean(-1).exp()
    ...
    while config['n_epochs'] == -1 or step // epoch_size < config['n_epochs']:
        ...
        for batch_idx in tqdm(
            ...
            loss, new_chunk_size = lib.deep.zero_grad_forward_backward(
                optimizer,
                lambda idx: loss_fn(apply_model('train', idx).exp(), Y_train[idx].unsqueeze(1).repeat(1, K)),
                batch_idx,
