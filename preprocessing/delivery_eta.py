...
def main(original_data_path: str | None):
    ...
    if original_data_path is None:
        ...
        if (TMP_DATA_PATH / "delivery_eta.parquet").exists():
            logger.info(f"Skip downloading, found files at {TMP_DATA_PATH}")
        else:
            kaggle.api.dataset_download_files(
                "pcovkrd84mejm/delivery-eta", path=TMP_DATA_PATH
            )
    ...
    data = pl.read_parquet(TMP_DATA_PATH / "delivery_eta.parquet").with_row_index(
        name="index_in_full"
    )

    bin_cols = [c for c in data.columns if c.startswith("bin")]
    cat_cols = [
        c for c in data.columns if c.startswith("cat") and c not in ["cat_1", "cat_2"]
    ]
    num_cols = [
        c
    ...
    default_train_idx = (
        data.with_row_index()
        .filter(pl.col("timestamp").lt(pl.datetime(2023, 12, 11, 0, 0, 0)))["index"]
        .to_numpy()
    )
