...
def main(original_data_path: str | None):
    ...
    X_bin_data = data.select(
        *[
            pl.col(f"has_bought_{c}").eq(0).alias(f"never_bought_{c}")
            for c in ["company", "category", "brand"]
        ],
        pl.col("has_bought_brand")
        .ne(0)
        .and_(pl.col("has_bought_category").ne(0))
        .and_(pl.col("has_bought_company").ne(0))
        .alias("has_bought_brand_company_category"),
        pl.col("has_bought_brand")
        .ne(0)
        .and_(pl.col("has_bought_category").ne(0))
        .alias("has_bought_brand_category"),
        pl.col("has_bought_brand")
        .ne(0)
        .and_(pl.col("has_bought_company").ne(0))
        .alias("has_bought_brand_company"),
    ).cast(pl.Float32)
    ...
    default_test_idx = (
        data.with_row_index()
        .filter(pl.col("offerdate").ge(pl.datetime(2013, 4, 25)))["index"]
        .to_numpy()
    )
    ...
    default_train_idx = (
        data.with_row_index()
        .filter(pl.col("offerdate").lt(pl.datetime(2013, 4, 20)))["index"]
        .to_numpy()
    )
    ...
    def time_range(df, split):
        ...
        return (
            df[split[-1].item()]["offerdate"].item()
            - df[split[0].item()]["offerdate"].item()
        )
