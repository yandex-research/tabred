...
def main(original_data_path: str | None):
    ...
    data = data.with_columns(
        pl.col("Original_Quote_Date").str.to_datetime().alias("timestamp")
    )
    ...
    data = data.with_columns(
        pl.col([c for c in data.columns if c.endswith(("A", "B"))]).replace({-1: None})
    )
    ...
    all_nulls = [
        k
        for k, v in data.select(pl.col("*").is_null().mean().eq(1.0))
        .to_dicts()[0]
        .items()
        if v
    ]
    ...
    bin_columns = [
        ...
        if count == 2
        if c not in target_columns + timestamp_meta_columns
    ]
    X_bin_data = (
        data[bin_columns]
        .with_columns(
            # ordinals with only 1 non null value
            pl.col(col.name for col in data[bin_columns].null_count() if col.item() > 0)
            .is_null()
            .not_(),
            # N/Y binary columns
            pl.col(col for col in bin_columns if data.schema[col] == pl.String).replace(
                {"N": 0, "Y": 1}
            ),
    ...
    cat_columns = [
        ...
        if dtype == pl.String
        and c not in bin_columns + target_columns + timestamp_meta_columns
    ]
    ...
    num_columns = list(
        set(data.columns)
        - set(bin_columns + cat_columns + target_columns + timestamp_meta_columns)
    )
    ...
    def time_range(df, split):
        ...
        return (
            df[split[-1].item()]["timestamp"].item()
            - df[split[0].item()]["timestamp"].item()
        )
