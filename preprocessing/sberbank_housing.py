...
def main(original_data_path: str | None):
    ...
    data_macro = (
        ...
        .with_columns(
            ...
            pl.col("child_on_acc_pre_school")
            .str.replace(",", ".")
    ...
    bin_cols = [
        data.columns[i]
        for i in np.flatnonzero(data.select(pl.col("*").n_unique() == 2).to_numpy())
    ]
    ...
    X_cat_data_np = OrdinalEncoder(
        min_frequency=1 / 100, encoded_missing_value=-1
    ).fit_transform(X_cat_data_np)
    ...
    default_test_idx = (
        data.with_row_index()
        .filter(pl.col("timestamp").ge(pl.datetime(2014, 12, 1)))["index"]
        .to_numpy()
    )
    ...
    default_train_idx = (
        data.with_row_index()
        .filter(pl.col("timestamp").lt(pl.datetime(2014, 6, 30)))["index"]
        .to_numpy()
    )
