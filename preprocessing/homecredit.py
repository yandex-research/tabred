...
def main(original_data_path: str | None):
    ...
    train_basetable = read_tables(DATA_FILES_PATH.glob("train_base.parquet")).pipe(
        set_table_dtypes
    )
    train_static = read_tables(DATA_FILES_PATH.glob("train_static_0_*.parquet")).pipe(
        set_table_dtypes
    )
    train_static_cb = read_tables(
        DATA_FILES_PATH.glob("train_static_cb_0.parquet")
    ...
    train_aggregated = list(
        itertools.chain.from_iterable(
            [
                aggregate_features(
                    read_tables(DATA_FILES_PATH.glob(name)).pipe(set_table_dtypes)
                )
                for name in [
    ...
    drop_cols = [
        c
        for c, dtype in data.schema.items()
        if (
            many_nulls[c].item()
            or n_unique[c].item() == 1
            or (dtype == pl.String and n_unique[c].item() > 50)
        )
    ]
    ...
    cat_cols = [
        ...
        if (
            c
            not in [n.meta.output_name() for n in bin_cols]
    ...
    num_cols = [
        ...
        if (
            ...
            not in [n.meta.output_name() for n in bin_cols]
            + ["target", "case_id", "date_decision"]
            and dtype in [pl.Float32, pl.Float64, pl.UInt32, pl.Boolean]
        )
    ]
    ...
    X_num_data = data_sample.select(
        num_cols + ["day_of_week", "day_of_month", "day_of_year"]
    ).cast(pl.Float32)
    ...
    default_train_idx = (
        data_sample.with_row_index()
        .filter(pl.col("date_decision").lt(pl.datetime(2020, 1, 1, 0, 0, 0)))["index"]
        .to_numpy()
    )
    ...
    default_test_idx = (
        data_sample.with_row_index()
        .filter(pl.col("date_decision").ge(pl.datetime(2020, 5, 1, 0, 0, 0)))["index"]
        .to_numpy()
    )
    ...
    train_size = (
        data_sample.shape[0] - (num_splits - 1) * test_val_size - 2 * test_val_size
    )
    ...
    sliding_window_splits = [
        {
            ...
            "test_idx": np.arange(
                train_size + test_val_size, train_size + 2 * test_val_size
            ),
        }
    ]
    ...
    def time_range(df, split):
        ...
        return (
            df[split[-1].item()]["date_decision"].item()
            - df[split[0].item()]["date_decision"].item()
        )
    ...
    all_splits = (
        ...
        | {
            f"sliding-window-{i}": split
            for i, split in enumerate(sliding_window_splits)
        }
